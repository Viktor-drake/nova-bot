...
// timeoutMs=20000 — чтобы зависший Notion-запрос не съел весь maxDuration функции
const notion = new Client({ auth: process.env.NOTION_TOKEN, timeoutMs: 20_000 });
...
// --- Dialog storage: ONE Notion page per chatId, full transcript kept as JSON in "Текст" ---
// (было: 1 страница на каждое сообщение — это и съедало лимит блоков на free-плане)
const DIALOG_MAX_MESSAGES = 80; // защита от разрастания property сверх разумного

function chunkRichText(str) {
  const chunks = [];
  for (let i = 0; i < str.length; i += 1900) {
    chunks.push({ text: { content: str.slice(i, i + 1900) } });
  }
  return chunks.length ? chunks : [{ text: { content: "" } }];
}

async function findDialogPage(chatId) {
  if (!DB.dialogs) return null;
  const res = await notion.databases.query({
    database_id: DB.dialogs,
    filter: { property: "Chat ID", rich_text: { equals: String(chatId) } },
    page_size: 1,
  });
...
// --- Append a message to the user's single dialog record (creates it on first message) ---
async function saveMessage(chatId, role, text, participantId) {
  if (!DB.dialogs) return;
  const existing = await findDialogPage(chatId);
  const transcript = existing ? readTranscript(existing) : [];
  transcript.push({ role, text, ts: new Date().toISOString() });
  if (transcript.length > DIALOG_MAX_MESSAGES) {
    transcript.splice(0, transcript.length - DIALOG_MAX_MESSAGES);
  }
  const richText = chunkRichText(JSON.stringify(transcript));
  const properties = {
    Текст: { title: richText },
...
// --- Load full dialog history for a user (single page, no pagination needed) ---
async function getRecentMessages(chatId, limit = 20) {
  if (!DB.dialogs) return [];
  let lastErr = null;
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const page = await findDialogPage(chatId);
      if (!page) return [];
      const transcript = readTranscript(page);
      return transcript.slice(-limit).map((m) => ({
        role: m.role === "assistant" ? "assistant" : "user",
        content: m.text || "",
      }));
    } catch (e) {
      lastErr = e;
      const isTimeout = (e.message || "").toLowerCase().includes("timed out") || (e.message || "").toLowerCase().includes("timeout") || e.code === "notionhq_client_request_timeout";
      if (!isTimeout || attempt === 1) throw e;
      console.warn(`[notion] getRecentMessages timed out, retry ${attempt + 1}`);
...
// --- Clear a user's dialog record (only used for explicit manual cleanup, not called after FINISH anymore) ---
async function deleteUserDialogs(chatId) {
  const page = await findDialogPage(chatId);
  if (!page) return 0;
...
// Принудительно сбросить кэш — вызывается после анкеты, чтобы свежие ресурсы участника попали в snapshot
function invalidateSnapshotCache() {
  snapshotCache = { text: null, ts: 0 };
}
