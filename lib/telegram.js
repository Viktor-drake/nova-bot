...
// --- Send voice message (ogg/opus buffer) ---
async function sendVoice(chatId, oggBuffer) {
  const form = new FormData();
  form.append("chat_id", String(chatId));
  form.append("voice", new Blob([oggBuffer], { type: "audio/ogg" }), "voice.ogg");
  const res = await fetch(`${BOT_URL()}/sendVoice`, { method: "POST", body: form });
  const data = await res.json();
  if (!data.ok) throw new Error(`Telegram sendVoice failed: ${data.description || JSON.stringify(data)}`);
  return data;
}

module.exports = { sendMessage, sendTyping, sendVoice, downloadFile, setWebhook };
