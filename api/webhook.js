...
КАК РАБОТАЕТ ЭТОТ БОТ (техническая реальность):
Голосовой pipeline полностью автоматический: голосовое пользователя → Whisper расшифровывает в текст → ты читаешь текст и пишешь ответ → TTS shimmer озвучивает твой текст → пользователь получает аудио.
Ты пишешь текст — система превращает его в голос. Это уже происходит прямо сейчас.
ПРАВИЛО: никогда не объясняй пользователю эту техническую цепочку и не комментируй как работает голос. Если спросят — скажи "да, всё работает" и перейди к делу. Кнопка "🔇 Голос: вкл/выкл" управляет озвучкой.
  ...
  try {
    ...
    if (userText === BUTTONS.FINISH) {
      ...
      try {
        ...
        // Запись диалога больше НЕ удаляется: это одна консолидированная страница
        // на пользователя (не по сообщению), полный текст анкеты остаётся как история.
